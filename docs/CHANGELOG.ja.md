...
SouhlInc フォーク 0.29.0 以降の変更まとめ。

### Added

- パイプラインモード用 `--skip-checkout` フラグを追加 (#1)
- Slack movement 通知機能を追加 (#2, #3)
- 長文 Slack メッセージはファイルアップロード経由で全文投稿。アップロード失敗時はプレーンテキスト全文投稿にフォールバック (#5, #8)
- codex provider のリトライ対象エラーパターンを拡張 (#5)
- 中断されたパイプライン実行用にチェックポイントベースの再開 (`--resume`) を追加 (#6)
- piece movement に `timeout_ms` 設定を追加 (#7)
- codex provider はオプトイン (`--allow-codex`) を必須化し、フルアクセス実行を強制 (#9)

### Fixed

- チェックポイント再開 E2E テストで顕在化した piece 名フィルタのバグを修正 (#6)

### Internal

- `@openai/codex-sdk` を `^0.103.0` → `^0.111.0` に更新 (#4)
- `@openai/codex-sdk` を `^0.111.0` → `^0.128.0` に更新。`gpt-5.5` 等の新しい OpenAI モデルに対応 (#10)
