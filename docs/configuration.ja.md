...
Codex の実行はコマンドごとの明示許可が必要です。`provider: codex`、`--provider codex`、movement レベルの `provider: codex`、または `persona_providers` によって movement が Codex に解決される場合、コマンドに `--allow-codex` を付けないと TAKT は Claude に fallback します。
...
|--------|------|--------|-------|----------|--------------|---------|
| `readonly` | 読み取り専用、ファイル変更不可 | `default` | `danger-full-access` + `approval never` | `read-only` | デフォルトフラグ（`--force` なし） | フラグなし |
| `edit` | 確認付きでファイル編集を許可 | `acceptEdits` | `danger-full-access` + `approval never` | `workspace-write` | デフォルトフラグ（`--force` なし） | `--allow-all-tools --no-ask-user` |
| `full` | すべてのパーミッションチェックをバイパス | `bypassPermissions` | `danger-full-access` + `approval never` | `danger-full-access` | `--force` | `--yolo` |

Codex は、ビルドやテストツールとの互換性を優先するため、常に `sandboxMode: "danger-full-access"` と `approvalPolicy: "never"` で起動します。Provider プロファイルは他 provider の permission mode 解決には使われますが、Codex の sandbox は permission mode によって弱められません。
...
いずれかの persona ルートを Codex で実行したい場合は、`--allow-codex` を付けて実行してください。
