...
Codex の実行はコマンドごとの明示許可が必要です。`provider: codex`、`--provider codex`、movement レベルの `provider: codex`、または `persona_providers` によって movement が Codex に解決される場合、コマンドに `--allow-codex` を付けないと TAKT は Codex 呼び出し前に中断します。
...
いずれかの persona ルートが Codex に解決される場合は、`--allow-codex` を付けて実行してください。
